...
if ($role === 'petugas') {
    $query = 'SELECT * FROM laporan_kehilangan ORDER BY created_at DESC LIMIT ' . $items_per_page . ' OFFSET ' . $offset;
    $count_query = 'SELECT COUNT(*) as total FROM laporan_kehilangan';
} else {
    ...
    $query = 'SELECT * FROM laporan_kehilangan WHERE created_by = ' . $user['id'] . ' ORDER BY created_at DESC LIMIT ' . $items_per_page . ' OFFSET ' . $offset;
    $count_query = 'SELECT COUNT(*) as total FROM laporan_kehilangan WHERE created_by = ' . $user['id'];
}
...
<head>
    ...
    <title>Laporan Kehilangan - TemuBalik</title>
    ...
    <main class="main-content">
        ...
        <section class="page-header">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h1 class="page-title">📋 Laporan Kehilangan</h1>
                    <p class="page-subtitle"><?php echo ($role === 'petugas') ? 'Kelola semua laporan kehilangan yang masuk' : 'Laporan kehilangan barang Anda'; ?></p>
                </div>
                <div class="col-md-4 text-md-end mt-3 mt-md-0">
                    <?php if ($role === 'pelapor'): ?>
                    <a href="create.php" class="btn btn-primary-custom d-inline-flex align-items-center gap-2 text-decoration-none">
                        <i data-lucide="plus" width="18"></i> Laporan Baru
                    </a>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="filter-container">
            ...
            <div class="search-bar ms-md-auto" style="max-width: 320px;">
                ...
                <input type="text" id="filterSearch" placeholder="Cari laporan...">
            </div>
        </div>
            ...
            <div class="item-list">
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): 
                        ...
                        if($row['status'] == 'dilaporkan') $status_class = 'pill-danger';
                        if($row['status'] == 'ditemukan') $status_class = 'pill-warning';
                        if($row['status'] == 'diserahkan' || $row['status'] == 'ditutup') $status_class = 'pill-success';
                    ?>
                    <div class="item-row">
                        <div class="item-img">
                            <?php if (!empty($row['foto'])): ?>
                                <img src="../uploads/<?= htmlspecialchars($row['foto']) ?>" alt="Foto Laporan" class="item-photo">
                            <?php else: ?>
                        ...
                        <div class="flex-grow-1">
                            ...
                            <p class="text-slate small mb-0">
                                ...
                                <span><?= htmlspecialchars($row['lokasi_hilang']) ?></span>
                            </p>
                        </div>
                ...
                <?php else: ?>
                    <div class="empty-state-box">
                        ...
                        <p class="empty-state-description">Tidak ada laporan kehilangan yang terdaftar di database.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        function filterTable() {
            ...
            if (!emptyStateBox && visibleCount === 0 && (status || kategori || keyword)) {
                ...
                emptyStateBox.innerHTML = `
                    ...
                    <p class="empty-state-description">Tidak ada laporan yang cocok dengan filter pencarian Anda.</p>
                `;
